...
block_batt = sparse([[block_eye],[block_zerovec]]) - sparse([[block_zerovec],[block_eye]])
#print(block_eye)
#print(block_zeros)
#print(block_batt)
...
Aeq_balance = sparse([[block_eye], [block_eye], [-block_eye], [block_zeros],[block_zerovec], [-block_eye], [pvoutput],[wtoutput],[block_zerovec]])
#print(Aeq_balance)
beq_balance = matrix(demand_island[0:t],tc='d')

Aeq_batterydynamic = sparse([[block_zeros],[-1/eff_batt*block_eye],[eff_batt*block_eye],[block_batt],[block_zeros],[block_zerovec],[block_zerovec],[block_zerovec]])
#print(Aeq_batterydynamic)
beq_batterydynamic = block_zerovec

Aeq_batteryinitial = sparse([[block_zerovec.T],[block_zerovec.T],[block_zerovec.T],[matrix([1])],[block_zerovec.T],[block_zerovec.T],[sparse([[0],[0],[0]])]])
beq_batteryinitial = matrix([0])

A_maximumstored = sparse([[block_zeros],[block_zeros],[block_zeros],[block_zerovec],[block_eye],[block_zeros],[block_zerovec],[block_zerovec],[-block_onevec]])
b_maximumstored = block_zerovec
#print(A_maximumstored)

A_penetration = sparse([[block_onevec.T],[block_zerovec.T],[block_zerovec.T],[block_zerovec.T],[block_zerovec.T],[sparse(matrix(np.zeros((1,4))))]])
...
c = matrix([[cost_diesel*block_onevec.T],[eps*block_onevec.T],[eps*block_onevec.T],[matrix([0])],[block_zerovec.T],[block_zerovec.T],[matrix([cost_cap[0]])],[matrix([cost_cap[1]])],[matrix([cost_cap[5]])]]) # 이거 sparse로 하면 셧다운되는데 matrix로 하면 잘 됨...왜지..
#print(c)
...
print('태양광 {}kW, 풍력 {}기, 배터리 {}kWh입니다'.format(round(x[43801]),round(x[43802]),round(x[43803])))
